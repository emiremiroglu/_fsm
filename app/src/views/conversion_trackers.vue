  ...
  <section v-if="!loading" class="p-6">
    <div class="flex justify-between">
      ...
      <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
        <router-link to="/conversion_trackers/create" class="button">New Tracker</router-link>
      </div>
    </div>
    <div class="min-w-full h-screen overflow-scroll">
      <table class="text divide-y w-full divide-zinc-300 dark:divide-zinc-700">
        <thead>
          <tr>
            <th>Status</th>
            <th>ID</th>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-zinc-800">
          <template v-for="tracker in trackers">
            <tr class="dark:hover:bg-zinc-800/50">
              <td>
                <div v-if="tracker.status" class="w-6 h-3 rounded-full bg-green-500"></div>
                <div v-if="!tracker.status" class="w-6 h-3 rounded-full bg-zinc-300 dark:bg-zinc-800"></div>
              </td>
              <td>
...
<style scoped>
  table { @apply mb-10; }
  table th { @apply text-left font-semibold py-2 px-2; }
